...
- Comeback gift: returning after `7` or more days away grants `Duck Feed x2` and `Treat x1` on load, framed as the ducks missing the player
...
- Returning after `7` or more days away grants a comeback gift on load (`Duck Feed x2`, `Treat x1`), derived from the same server-owned last-seen timestamp as offline progress but uncapped. It is a welcome, never a recovered loss, per the cozy promise.
- Join-time rewards (offline eggs, the daily streak claim, and the comeback gift) present together in one tap-to-dismiss `Welcome back!` card on the farm instead of separate floating toasts. The card is suppressed during fresh-profile onboarding and while visiting another farm; the same rewards arriving mid-session fall back to floating toasts.
- Fresh profiles reveal farm controls through the Feature Unlock Ladder: Collect, Sell, and counters from the start; the quest tracker and `Upgrades` after the first sell; `Shop` once the second duck is owned; and `Games`/`Visit` after the first duck-help action. Rungs derive from server-owned progress, locked actions are rejected server-side, nothing re-locks, and profiles saved before the ladder shipped keep every feature.
