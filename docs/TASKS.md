...
- [ ] Wire Phase 6 egg reward sources (per ROADMAP): add `Speckled Egg ×1` to the Daily Quests all-complete bonus and the Comeback Gift package, with the granting toast/notice copy updated to surface the egg. Consider modest egg drops from Egg Catch thresholds and quest milestones once the core loop is play-tested. Eggs remain never-for-Robux.
- [ ] Implement Phase 6 client UI: an `Eggs` surface (third tab in the Duck Shop panel) to buy eggs with visible odds before purchase, incubator slot(s) on the farm nest area showing the hatch countdown and a `Collect` action on finish, the wiggle-crack-reveal hatch moment driven by the `egg_hatched` notice, and a `hatching` first-use guide step per the no-system-ships-untaught rule. Keep the farm-screen secondary-control count within the `6` budget (reuse panels/nest, no new permanent button).
