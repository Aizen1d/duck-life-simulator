...
- Returning after `7` or more days away grants a comeback gift on load (`Duck Feed x2`, `Treat x1`) with a `Your ducks missed you!` toast, derived from the same server-owned last-seen timestamp as offline progress but uncapped. It is a welcome, never a recovered loss, per the cozy promise.
