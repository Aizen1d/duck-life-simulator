...
- Implemented the Phase 6 hatching client UI and egg reward sources (2026-06-13), making the loop fully playable. The Duck Shop panel's tab bar became three-way (`Families` / `Duckdex` / `Eggs`); the `Eggs` tab shows the incubator slot (empty / live `M:SS` countdown / `🐣 ready` with `Collect`) and one row per egg with its visible odds, hatch time, owned count, a `Buy <cost>` button, and an `Incubate` button gated on owning the egg and a free slot. Buying/incubating/collecting fire the `buy_egg` / `start_incubation` / `collect_hatch` actions; the `egg_hatched` notice shows a hatch-reveal toast; and a `hatching` first-use guide step joins the sequence (no new farm-screen control — reuses the existing panel, within the 6-secondary budget). Egg reward sources added per the roadmap: a config-driven Speckled Egg now joins the Daily Quests all-complete bonus and the Comeback Gift, surfaced by display name in the daily-bonus toast and the Welcome Back card. The incubator lives in the panel for V0; moving it onto the nest scene with the wiggle-crack-reveal animation is a deferred visual polish.

- Implemented the Phase 6 core hatching loop server logic (2026-06-13): schema bumped to `13`; `eggInventory`, `incubatorSlots`, and `hatchPity` persist and load with safe defaults, integer clamping, known-egg-type and finite-`finishUtc` validation, and slot-count capping (a finished-while-away egg loads collectable). Three new actions — `buy_egg` (coin sink tagged `econ_sink_egg_<type>`), `start_incubation` (validates an owned egg and a free slot; writes a server-derived `finishUtc`), and `collect_hatch` (rejects unfinished slots; rolls rarity from the egg odds; applies pity by forcing `pityFloor` after `pityThreshold` sub-floor hatches and resetting on a floor-or-better roll; maps rarity to a family via the extended `pickFamilyForRarity` now covering common/legendary; grants the duck; fires an `egg_hatched` notice and `funnel_first_hatch`). Egg inventory and per-slot remaining seconds replicate via `serializeIncubatorSlots`. Visitor actions on another farm stay blocked by the existing farm-view guard. New actions/notice added to `RemoteProtocol.luau`. Egg reward sources (Speckled Egg in the daily bonus and Comeback Gift) and the client UI are the remaining Phase 6 steps.

- Ran the Phase 6 Hatching design pass and implemented its data layer (2026-06-13): locked egg types (`Plain` 50 coins / 5 min, `Speckled` 150 / 30 min, `Golden` 600 / 4 h), their hatch odds (Plain Common `75`/Uncommon `25`; Speckled Uncommon `60`/Rare `30`/Epic `10`; Golden Rare `55`/Epic `35`/Legendary `10`), and pity (Speckled guarantees Rare+ within `8`, Golden guarantees Legendary within `10`). Resolved the roadmap's `60/30/9/1` "Legendary-excluded" Speckled ambiguity in favor of `60/30/10` so Legendary stays gated behind the premium Golden Egg, and set the previously-unspecified Plain Egg odds and egg coin prices — all flagged as tunable starter numbers. Added `eggTypes` and `incubator` config tables to `PrototypeConfig.luau`, `EggTypeDefinition`/`IncubatorDefinition` shared types, and documented the schema-13 save shape (`eggInventory`, `incubatorSlots`, `hatchPity`) with the at-collect server-roll and forged-timestamp rules in `SAVE_DATA_DESIGN.md`. Decisions and the remaining server-logic/client-UI steps are recorded in `docs/TASKS.md`. No server or client wiring yet; eggs are never sold for Robux (hard rule preserved).

- Wired the Mystery Duck Box Open affordance into the client (2026-06-13): the Duckdex tab of the Buy Duck panel now shows a highlighted `🎁 Mystery Duck Box ×N` row with an `Open` button at the top whenever the player owns at least one box (granted by Duckdex collection milestones). Pressing `Open` fires `open_mystery_duck_box`; the server replies with three rarity-weighted candidates that the existing pick panel (ZIndex ui+30+, above the Buy Duck panel) displays for the player to choose one. The Duckdex tab label gains a `🎁` hint while boxes are unopened, and the `collection_reward_granted` toast now points the player to `Buy Duck → Duckdex` when a box is part of the reward. Closes the loop on Phase 5: boxes earned from milestones were previously unopenable from the UI.

- Implemented `AnalyticsService` V0 (2026-06-13): a server-only module (`src/server/AnalyticsService.luau`) wrapping the Roblox built-in `AnalyticsService` for funnel and economy instrumentation per the Product Plan. Every call is `task.spawn` + `pcall` guarded and gated by a kill-switch flag, so analytics can never break gameplay. Wired economy source/sink events at every coin-mutation site (sources: `egg_sale`, `quest_reward`, `daily_quest_reward`, `daily_quest_bonus`, `daily_checkin`, `minigame_egg_catch`, `collection_reward`; sinks: `egg_value_upgrade`, `duck_purchase`, `shop_<itemId>`) and one-time funnel/social milestones (`funnel_first_collect/sell/buy_duck/care/shop_purchase/minigame`, `social_first_visit`). Events queue on `state.pendingAnalytics` and flush in `sendState`, which holds the owning `Player` the API requires — mirroring the existing `pendingNotices` deferral. First-moment events fire once per profile lifetime, deduped by a new persisted `funnelMilestones` set (`schemaVersion = 12`). The `funnel_*`/`econ_source_*`/`econ_sink_*`/`social_*` naming convention is recorded in `docs/CODING_STANDARDS.md`.

- Implemented Buy Duck V1 family-selector + Duckdex tab (2026-06-13): `createBuyDuckFamilyPanel` renders a floating panel with two tabs — Families shows all 7 families with coin costs and Buy buttons (hatch-only families show a `Hatch-only` label and are disabled), Duckdex shows a collection-% header, per-family ownership indicators, and the 4 milestone reward rows. The panel opens from the `BuyDuck` progression card (now labeled `Browse`); the tutorial fast-path still fires the direct `buy_duck` action for classic_yellow when the tutorial `buy_duck` step is active. Buying a family fires `buy_duck { familyId }` and closes the panel. Backdrop click closes. Panel uses `ScrollingFrame` content areas for both tabs. ZIndex anchored at ui+11/12/13, positioned above the progression stack.
...
- Removed the rarity badges from the Starter Choice cards (2026-06-13): the first-duck pick now shows just the family name and a `Choose` button, so no option looks mechanically "better" at the one moment a new player can't yet read rarity (the `families` guide step that explains it fires right after). Found during owner play-testing of the Phase 5 starter overlay. The underlying rarities are unchanged — Mallard Green stays Common (`+0%`) and Choco Brown / Snowy White stay Uncommon (`+5%`); the gap is intentionally a small generous-starter perk rather than a visible trade-off, and the now-empty middle of each card is reserved for the family sprite once the `Duck Family Lineup Sheet` art lands.
...
- Fixed `buy_duck` crashing and silently losing coins (2026-06-13): `buyDuck` called `grantDuckWithFamily` ~650 lines before that function was defined, so the name resolved to `nil` and threw `attempt to call a nil value` — after the coin cost had already been deducted, leaving the player paying for a duck they never received. This broke every duck purchase (family path, the legacy classic path, and the tutorial fast-path), which also blocked the second-duck Feature Unlock Ladder rung and the guide's Buy Duck step. Fixed with a forward declaration of `grantDuckWithFamily` above `buyDuck`. `rojo build` is clean and the build never caught it because Luau resolves globals at call time, not parse time. Found and re-verified via the Roblox Studio MCP: after the fix, buying a third family now correctly fires the 50% Duckdex milestone (granting a Mystery Box), and the box open/pick flow completes. Server logic for Phase 5 (families, rarity, Starter Choice, collection milestones, Mystery Duck Box) verified live with no Output errors.
