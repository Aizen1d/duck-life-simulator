...
- [ ] Implement Feature Unlock Ladder V0 for fresh profiles per the comprehension guardrails in `docs/ROADMAP.md`: Collect/Sell first, quest tracker and `Upgrades` after the first sell, `Shop` with its guide step, `Minigames`/`Visit`/streak/dailies after guide completion, unlock state derived from existing server progress, nothing re-locks, and existing profiles keep everything.
