...
- Save Data V0 is implemented and documented in `docs/SAVE_DATA_DESIGN.md`; the current saved schema is version `10`, which adds the Feature Unlock Ladder flag, Daily Quests V0 progress, the Daily Streak V1 `streakDay` position, and the server-derived `lastSeenUtc` timestamp for Offline Progress V0 (offline egg accrual at `50%` of the online rate, capped at `2` hours) on top of the inventory, quest, and daily check-in fields from earlier versions. Studio DataStore testing requires API services to stay enabled for this place. The farm screen does not show a save status indicator; verify saves with Studio Output logs and stop/rejoin checks.
