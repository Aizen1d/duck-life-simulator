...
- [x] Wire the Mystery Duck Box Open affordance into the client (2026-06-13): the Duckdex tab shows a `🎁 Mystery Duck Box ×N` row with an `Open` button when `farmState.mysteryDuckBoxes > 0`; pressing it fires `open_mystery_duck_box`, and the server-replicated `pendingDuckBoxCandidates` drives the existing pick panel. Duckdex tab label gains a `🎁` hint while boxes are unopened, and the collection-reward toast points to `Buy Duck → Duckdex` when a box drops. Closes the previously-missing UI entry point for opening boxes earned from Duckdex milestones.
- [ ] Phase 5 visuals (Play-mode eyes): the Buy Duck family-selector panel and Duckdex tab render with hatch-only rows disabled, the Starter Choice overlay opens after guide completion, the Mystery Duck Box `Open` button (Duckdex tab, shown only when a box is owned) fires the open action and the pick panel displays its three candidates above the Buy Duck panel, picking grants the duck and decrements the box count, and the `pending_starter_choice` / `collection_reward_granted` / `duck_box_candidate_picked` toasts show correctly.
- [x] Run the Phase 6 Hatching design pass before implementation (2026-06-13): all decisions recorded below; egg/incubator/hatch art (`Egg Type Icon Sheet`, `Incubator and Hatch Stage Sheet`, `Hatch Reveal Background`) is still pending owner generation but does not block the data layer or server logic.

  **Egg types (`schemaVersion = 13`):**
  | Egg | Buy cost | Hatch time | Odds | Pity |
  |---|---|---|---|---|
  | `Plain Egg` | 50 coins | 5 min (`300s`) | Common `75` / Uncommon `25` | none |
  | `Speckled Egg` | 150 coins | 30 min (`1800s`) | Uncommon `60` / Rare `30` / Epic `10` | Rare-or-better within `8` |
  | `Golden Egg` | 600 coins | 4 h (`14400s`) | Rare `55` / Epic `35` / Legendary `10` | Legendary within `10` |

  `Festival Egg` is reserved for the Phase 11 event framework and is not in V0. All odds are starter numbers to tune after first data.

  **Resolved roadmap ambiguities (decide-and-document, owner may veto):**
  - The roadmap wrote Speckled as `60/30/9/1` with a confusing "Legendary-excluded" note. Resolved in favor of `60/30/10` (Uncommon/Rare/Epic, no Legendary) so Legendary stays gated behind the premium Golden Egg and the Duckdex/event milestones — protecting Golden Egg's specialness and keeping the cheap-to-mid eggs honest. The spare `1%` folds into Epic.
  - The roadmap did not give Plain Egg odds; set to Common `75` / Uncommon `25` so even the entry egg has collection value without competing with the flat-priced Uncommon Buy Duck options.
  - Egg coin prices (`50/150/600`) are new: Plain undercuts a direct Uncommon duck buy (`90`) as a cheaper gamble; Speckled is the only sub-`200` path to Rare/Epic (timer-gated); Golden is the premium Legendary path. Eggs are also earned non-coin from quests/daily/minigames/events (wired with server logic). Hard rule preserved: eggs and odds are never sold for Robux.

  **Rarity → family mapping (reuses the Mystery Duck Box convention, extended for Common and Legendary):** Common → `classic_yellow`/`mallard_green` (50/50); Uncommon → `choco_brown`/`snowy_white` (50/50); Rare → `blossom_pink`; Epic → `twilight_blue`; Legendary → `golden`.

  **Incubator V0:** a single hatch slot (`incubator.slotCount = 1`); Phase 8 raises the count via farm level. The slot stores only the egg type id and a server-derived `finishUtc`. The hatch rarity is rolled server-side at collect time (not at start, not stored), so it cannot be inspected or save-scummed before the reveal. Pity counters (`hatchPity.speckled`/`hatchPity.golden`) increment per hatch of that egg type and reset when the roll lands on `pityFloor` or better; rarity rank is common < uncommon < rare < epic < legendary.

  **Hub consolidation note (UI surface budget):** the Hatchery must not add a permanent secondary control beyond the `6` cap. Per the roadmap, incubator slots live on the farm scene nest area itself; the buy-egg surface reuses the existing Buy Duck → "Duck Shop" panel as a third tab (`Eggs`) rather than a new button. To be confirmed during the client UI step.

  **Save schema 13:** `eggInventory` (`{plain, speckled, golden}`), `incubatorSlots` (array ≤ `slotCount`, each `{eggType, finishUtc}`), `hatchPity` (`{speckled, golden}`). Full shape and validation rules in `SAVE_DATA_DESIGN.md`.
- [x] Implement Phase 6 data layer (2026-06-13): added `eggTypes` (Plain/Speckled/Golden with buy cost, hatch seconds, odds, and pity threshold/floor) and `incubator` (`slotCount`) tables to `PrototypeConfig.luau`; added `EggTypeDefinition` and `IncubatorDefinition` shared types and extended the `PrototypeConfig` type; documented the schema-13 save shape (`eggInventory`, `incubatorSlots`, `hatchPity`) and migration rules in `SAVE_DATA_DESIGN.md`. No server/client wiring yet (next steps below).
- [x] Implement Phase 6 server logic — core hatching loop (2026-06-13): schema bumped to `13`; `eggInventory`, `incubatorSlots`, and `hatchPity` persist and load with safe defaults, non-negative-integer clamping, known-egg-type and finite-`finishUtc` validation, and slot-count capping (a past `finishUtc` on load is legitimate — the egg finished while away and is collectable). Three new actions wired in `RemoteProtocol.luau`: `buy_egg` (coin sink, `econ_sink_egg_<type>`, adds to inventory), `start_incubation` (validates an owned egg and a free slot, writes a server-derived `finishUtc = os.time() + hatchSeconds`), and `collect_hatch` (rejects an unfinished slot; rolls rarity from the egg odds in fixed rarity-rank order; applies pity — forces `pityFloor` after `pityThreshold` sub-floor hatches and resets on a floor-or-better roll; maps rarity to a family via the extended `pickFamilyForRarity`, now covering common and legendary; grants the duck through `grantDuckWithFamily`; fires an `egg_hatched` notice and `funnel_first_hatch`). Egg inventory and per-slot remaining seconds (`ready` flag) replicate in the farm state via `serializeIncubatorSlots`. Visitor actions on another farm are blocked by the existing pre-dispatch farm-view guard. Egg reward sources and client UI are the follow-ups below.
- [x] Wire Phase 6 egg reward sources (2026-06-13): added a config-driven `Speckled Egg ×1` to the Daily Quests all-complete bonus (`dailyQuests.allCompleteBonusEggType/Amount`) and the Comeback Gift package (`comebackGift.eggType/Amount`) via a new `grantEgg` server helper; the daily-bonus toast and the Welcome Back card / comeback fallback toast now surface the egg by display name. Modest egg drops from Egg Catch thresholds / quest milestones remain a possible later addition once the loop is play-tested. Eggs remain never-for-Robux.
- [x] Implement Phase 6 client UI (2026-06-13): refactored the Duck Shop panel's two-way tab into three (`Families` / `Duckdex` / `Eggs`). The `Eggs` tab shows the single incubator slot (empty / `🥚 name · M:SS` countdown / `🐣 ready` with a `Collect` button) and one row per egg type with its visible odds, hatch time, owned count, a `Buy <cost>` button, and an `Incubate` button (enabled only when an egg is owned and the slot is free). `buy_egg` / `start_incubation` / `collect_hatch` fire the server actions; the `egg_hatched` notice shows a hatch-reveal toast; a `hatching` info step joins the guide sequence (same pattern as `streak`/`quests`/`families`). No new farm-screen control — it reuses the existing Duck Shop panel, staying within the `6`-secondary budget. The incubator currently lives in the panel rather than on the nest scene (a visual polish deferral — see below).
- [ ] Phase 6 client UI — visuals + nest polish (Play-mode eyes): confirm the three-tab Duck Shop renders correctly on desktop/compact/short-landscape; the Eggs tab odds/countdown/owned read clearly; Buy/Incubate/Collect disable states are right; the hatch-reveal toast and the daily-bonus / Welcome Back egg lines show; and decide whether to move the incubator slot onto the farm nest area (the roadmap's intended placement) with the wiggle-crack-reveal animation, which the V0 panel approximates with a toast.
- [x] Phase 6 hatching — algorithm + config verified headless (2026-06-13): ported the exact `rollEggRarity` / pity `resolveHatchRarity` / `pickFamilyForRarity` logic and the committed egg odds into `scripts/hatch_sim_check.py` and ran `500k` hatches per egg. Confirmed all three odds tables sum to `100`; empirical distributions match config (Plain `75/25`); the pity guarantees hold exactly (longest sub-`rare` Speckled run = `7` ≤ `7`; longest sub-`legendary` Golden run = `9` ≤ `9`, i.e. Rare+ within `8` Speckled and Legendary within `10` Golden); and every rolled rarity maps to a real family. Balance note: because Golden's pity floor *is* the top rarity, pity lifts its effective Legendary rate from the `10%` base to ~`15%` (a Legendary roughly every `6-7` Golden eggs), while Speckled pity mostly converts unlucky Uncommons into Rares (Uncommon ~`59%`, Rare ~`31%`). Tunable if too generous. This is an algorithm/config check, not live-code or persistence verification (no Lua interpreter or Studio MCP available this session).
- [ ] Phase 6 hatching — verify the live server + persistence in Roblox Studio (Play-mode / MCP, not available this session): buying each egg deducts coins and adds inventory; incubation writes a server-derived `finishUtc`; a slot becomes collectable exactly at finish and after a stop/rejoin spanning the timer (offline hatch); `collect_hatch` rejects an unfinished slot and grants a duck on finish; `econ_sink_egg_*` and `funnel_first_hatch` fire; and pre-schema-13 saves migrate with an empty incubator and zero pity. The headless check above already covers the roll/pity math.
