...
- [ ] Implement Phase 6 server logic: schema bump to `13`; persist/load `eggInventory`, `incubatorSlots`, `hatchPity` with safe defaults and forged/future-`finishUtc` clamping; new actions to buy an egg (coin sink, `econ_sink_egg_<type>`), start incubation (validates a free slot and owned egg), and collect a finished hatch (server-rolls rarity with pity, maps to a family via the extended `pickFamilyForRarity`, grants the duck through `grantDuckWithFamily`, resets/increments pity, fires a hatch notice + `funnel_first_hatch`); wire egg sources from quests/daily/minigames/events; replicate egg inventory and per-slot remaining seconds in the farm state.
- [ ] Implement Phase 6 client UI: an `Eggs` surface (third tab in the Duck Shop panel) to buy eggs with visible odds before purchase, incubator slot(s) on the farm nest area showing the hatch countdown and a `Collect` action on finish, and the wiggle-crack-reveal hatch moment; add a `hatching` first-use guide step per the no-system-ships-untaught rule. Keep the farm-screen secondary-control count within the `6` budget (reuse panels/nest, no new permanent button).
