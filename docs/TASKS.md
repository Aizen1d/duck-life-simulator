...
- [ ] Phase 6 — verify in Roblox Studio (Play-mode / MCP): buying each egg deducts coins and adds inventory; incubation writes a server-derived finish time; a slot becomes collectable exactly at finish (and after a stop/rejoin spanning the timer); collecting grants a duck of a rarity consistent with the odds; pity forces a Rare+ within 8 Speckled and a Legendary within 10 Golden hatches; `econ_sink_egg_*` and `funnel_first_hatch` fire; and pre-schema-13 saves migrate with an empty incubator and zero pity.
